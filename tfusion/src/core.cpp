...
int tf::cuda::getCudaEnabledDeviceCount()
{
    int count;
...
void tf::cuda::setDevice(int device)
{
    cudaSafeCall( cudaSetDevice( device ) );
}

std::string tf::cuda::getDeviceName(int device)
{
    cudaDeviceProp prop;
...
bool tf::cuda::checkIfPreFermiGPU(int device)
{
  if (device < 0)
...
namespace 
{
    template <class T> inline void getCudaAttribute(T *attribute, CUdevice_attribute device_attribute, int device)
    {
        *attribute = T();
        ...
        tfusion::cuda::error("driver API error", __FILE__, __LINE__);
    }
...
void tf::cuda::printCudaDeviceInfo(int device)
{
...
void tf::cuda::printShortCudaDeviceInfo(int device)
{
    int count = getCudaEnabledDeviceCount();
    bool valid = (device >= 0) && (device < count);
...
tf::SampledScopeTime::SampledScopeTime(double& time_ms) : time_ms_(time_ms)
{
    start = (double)cv::getTickCount();
}
tf::SampledScopeTime::~SampledScopeTime()
{
    static int i_ = 0;
    time_ms_ += getTime ();
    if (i_ % EACH == 0 && i_)
...
double tf::SampledScopeTime::getTime()
{
    return ((double)cv::getTickCount() - start)*1000.0/cv::getTickFrequency();
}

tf::ScopeTime::ScopeTime(const char *name_) : name(name_)
{
    start = (double)cv::getTickCount();
}
tf::ScopeTime::~ScopeTime()
{
