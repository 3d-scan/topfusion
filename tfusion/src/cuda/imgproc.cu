...
void tfusion::device::bilateralFilter (const Depth& src, Depth& dst, int kernel_size, float sigma_spatial, float sigma_depth)
{
...
void tfusion::device::truncateDepth(Depth& depth, float max_dist /*meters*/)
{
    dim3 block (32, 8);
...
void tfusion::device::depthPyr(const Depth& source, Depth& pyramid, float sigma_depth)
{
    sigma_depth *= 1000; // meters -> mm
...
void tfusion::device::computeNormalsAndMaskDepth(const Reprojector& reproj, Depth& depth, Normals& normals)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void tfusion::device::computePointNormals(const Reprojector& reproj, const Depth& depth, Points& points, Normals& normals)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void tfusion::device::compute_dists(const Depth& depth, Dists dists, float2 f, float2 c)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols (), block.x), divUp (depth.rows (), block.y));
...
void tfusion::device::resizeDepthNormals(const Depth& depth, const Normals& normals, Depth& depth_out, Normals& normals_out)
{
...
void tfusion::device::resizePointsNormals(const Points& points, const Normals& normals, Points& points_out, Normals& normals_out)
{
...
void tfusion::device::renderImage(const Depth& depth, const Normals& normals, const Reprojector& reproj, const float3& light_pose, Image& image)
{
    dim3 block (32, 8);
    dim3 grid (divUp (depth.cols(), block.x), divUp (depth.rows(), block.y));
...
void tfusion::device::renderImage(const Points& points, const Normals& normals, const Reprojector& reproj, const Vec3f& light_pose, Image& image)
{
    dim3 block (32, 8);
...
void tfusion::device::renderTangentColors(const Normals& normals, Image& image)
{
    dim3 block (32, 8);
...
void tfusion::device::mergePointNormal (const DeviceArray<Point>& cloud, const DeviceArray<float8>& normals, const DeviceArray<float12>& output)
{
    const int block = 256;
