...
#define REPORT_ERROR(msg) tfusion::cuda::error ((msg), __FILE__, __LINE__)

struct tfusion::OpenNISource::Impl
{
    Context context;
...
tfusion::OpenNISource::OpenNISource() : depth_focal_length_VGA (0.f), baseline (0.f),
    shadow_value (0), no_sample_value (0), pixelSize (0.0), max_depth (0) {}

tfusion::OpenNISource::OpenNISource(int device) {open (device); }
tfusion::OpenNISource::OpenNISource(const string& filename, bool repeat /*= false*/) {open (filename, repeat); }
tfusion::OpenNISource::~OpenNISource() { release (); }

void tfusion::OpenNISource::open (int device)
{
...
void tfusion::OpenNISource::open(const std::string& filename, bool repeat /*= false*/)
{
    impl_ = cv::Ptr<Impl> ( new Impl () );
...
void tfusion::OpenNISource::release ()
{
    if (impl_)
    {
...
bool tfusion::OpenNISource::grab(cv::Mat& depth, cv::Mat& image)
{
...
void tfusion::OpenNISource::getParams ()
{
    XnStatus rc = XN_STATUS_OK;
...
bool tfusion::OpenNISource::setRegistration (bool value)
{
    XnStatus rc = XN_STATUS_OK;
