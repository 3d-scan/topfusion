...
namespace tfusion
{
	...
	private:
		...
		HashSwapState *swapStates_host, *swapStates_device;
	...
	public:
		...
		HashSwapState *GetSwapStates(bool useGPU) { return useGPU ? swapStates_device : swapStates_host; }
		int *GetNeededEntryIDs(bool useGPU) { return useGPU ? neededEntryIDs_device : neededEntryIDs_host; }

		int noTotalEntries; 

		GlobalCache() : noTotalEntries(SDF_BUCKET_NUM + SDF_EXCESS_LIST_SIZE)
		{	
			...
			swapStates_host = (HashSwapState *)malloc(noTotalEntries * sizeof(HashSwapState));
			memset(swapStates_host, 0, sizeof(HashSwapState) * noTotalEntries);
			...
			ORcudaSafeCall(cudaMalloc((void**)&swapStates_device, noTotalEntries * sizeof(HashSwapState)));
			ORcudaSafeCall(cudaMemset(swapStates_device, 0, noTotalEntries * sizeof(HashSwapState)));
