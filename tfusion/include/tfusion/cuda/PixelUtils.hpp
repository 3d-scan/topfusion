...
template<typename T> _CPU_AND_GPU_CODE_ inline Vector4f interpolateBilinear(const CONSTPTR(tfusion::Vector4<T>) *source,
	const THREADPTR(Vector2f) & position, const CONSTPTR(Vector2i) & imgSize)
{
	...
	tfusion::Vector4<T> a = source[p.x + p.y * imgSize.x];
	tfusion::Vector4<T> b(T(0)), c(T(0)), d(T(0));
...
template<typename T> _CPU_AND_GPU_CODE_ inline Vector2f interpolateBilinear_Vector2(const CONSTPTR(tfusion::Vector2<T>) *source,
	const THREADPTR(Vector2f) & position, const CONSTPTR(Vector2i) & imgSize)
{
	const Vector2i p((int)floor(position.x), (int)floor(position.y));
	const Vector2f delta(position.x - (float)p.x, position.y - (float)p.y);

	tfusion::Vector2<T> a = source[p.x + p.y * imgSize.x];
	tfusion::Vector2<T> b(T(0)), c(T(0)), d(T(0));
...
template<typename T> _CPU_AND_GPU_CODE_ inline Vector4f interpolateBilinear_withHoles(const CONSTPTR(tfusion::Vector4<T>) *source,
	const THREADPTR(Vector2f) & position, const CONSTPTR(Vector2i) & imgSize)
{
	const Vector2s p((short)floor(position.x), (short)floor(position.y));
	const Vector2f delta(position.x - (float)p.x, position.y - (float)p.y);

	const tfusion::Vector4<T> a = source[p.x + p.y * imgSize.x];
	const tfusion::Vector4<T> b = source[(p.x + 1) + p.y * imgSize.x];
	const tfusion::Vector4<T> c = source[p.x + (p.y + 1) * imgSize.x];
	const tfusion::Vector4<T> d = source[(p.x + 1) + (p.y + 1) * imgSize.x];
