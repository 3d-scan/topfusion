...
namespace tfusion
{
    ...
    namespace device
    {
        using tfusion::cuda::DeviceMemory;
        using tfusion::cuda::DeviceMemory2D;
    }
}

/////////////////////  Inline implementations of DeviceMemory ////////////////////////////////////////////

template<class T> inline       T* tfusion::cuda::DeviceMemory::ptr()       { return (      T*)data_; }
template<class T> inline const T* tfusion::cuda::DeviceMemory::ptr() const { return (const T*)data_; }

template <class U> inline tfusion::cuda::DeviceMemory::operator tfusion::cuda::PtrSz<U>() const
{
    PtrSz<U> result;
    result.data = (U*)ptr<U>();
...
template<class T>        T* tfusion::cuda::DeviceMemory2D::ptr(int y_arg)       { return (      T*)((      char*)data_ + y_arg * step_); }
template<class T>  const T* tfusion::cuda::DeviceMemory2D::ptr(int y_arg) const { return (const T*)((const char*)data_ + y_arg * step_); }

template <class U> tfusion::cuda::DeviceMemory2D::operator tfusion::cuda::PtrStep<U>() const
{
    PtrStep<U> result;
    result.data = (U*)ptr<U>();
    result.step = step_;
...
template <class U> tfusion::cuda::DeviceMemory2D::operator tfusion::cuda::PtrStepSz<U>() const
{
    PtrStepSz<U> result;
    result.data = (U*)ptr<U>();
    result.step = step_;
