...
namespace tfusion
{
	/** \brief
	    ...
	    an Lib::Objects::Scene and fuse new image information
	    into them.
	*/
	...
	public:
		...
		*/
		virtual void AllocateSceneFromDepth(Scene<TVoxel, VoxelBlockHash> *scene, const Intr intr, 
	const Matrix4f pose, cuda::Dists &dist,const RenderState *renderState,bool onlyUpdateVisibleList = false,bool resetVisibleList = false) = 0;

		/** Update the voxel blocks by integrating depth and
		    possibly colour information from the given view.
		*/
		virtual void IntegrateIntoScene(Scene<TVoxel, VoxelBlockHash> *scene, const Intr intr,
	const Matrix4f pose, cuda::Dists& dist, const RenderState *renderState) = 0;
